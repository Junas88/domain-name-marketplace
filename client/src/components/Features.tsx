...
export default function Features() {
  ...
  const { data: featuresContent, isLoading } = useQuery<PageContent>({
    ...
    staleTime: 60 * 1000, // 1 minute - shorter stale time for homepage content
    refetchOnMount: true, // Always refetch when component mounts
  });

  // Function to render our custom green features that override any API-supplied DOM
  const renderGreenFeatures = () => (
    ...
    <section className="py-16 bg-white border-t border-black">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {isLoading ? (
          ...
          // Always use our green features implementation, ignore API content
          renderGreenFeatures()
        )}
      </div>
    </section>
